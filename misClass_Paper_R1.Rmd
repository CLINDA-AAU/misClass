...
Add colnames to cluster probabilities and merge to pData
```{r}
colnames(fit.GMM$z) <- row.names(conf.GMM)
pData.gmm <- merge(pData.gmm, fit.GMM$z, by.x = "Row.names", by.y = 0)
```
...
## Fuzzy cluster correction
Correction using fuzzy risk classes obtained by summing probabilities over high risk classes, so we get the probability of high risk
```{r}
pData.gmm$gmm.risk.fuzzy <- rowSums(pData.gmm[,c("PR", "MF", "MS")])
boxplot(pData.gmm$gmm.risk.fuzzy ~ pData.gmm$gmm.risk, xlab = "Risk Class", ylab = "Probability of risk class")
```

```{r}
cox.gmm.fuzzy <- coxph(OS ~ gmm.risk.fuzzy, data = pData.gmm)
cox.gmm.fuzzy
```
...
cox.gmm.fuzzy.sum <- summary(cox.gmm.fuzzy)

results <- matrix(0, nrow = 5, ncol =4)
# Zhan Naive
...
# Fuzzy
results[3,1] <- cox.gmm.fuzzy.sum$coefficients[2]
results[3,2] <- cox.gmm.fuzzy.sum$conf.int[3]
results[3,3] <- cox.gmm.fuzzy.sum$conf.int[4]
results[3,4] <- cox.gmm.fuzzy.sum$coefficients[5]

# Simex
results[4,1] <- exp(cox.gmm.simex$coefficients)[1]
results[4,2] <- exp(cox.gmm.simex$coefficients -1.96 * sqrt(cox.gmm.simex$variance.jackknife))
results[4,3] <- exp(cox.gmm.simex$coefficients +1.96 * sqrt(cox.gmm.simex$variance.jackknife))
results[4,4] <- cox.gmm.simex.sum$coefficients$jackknife[4]

# Simex bootstrap
df <- cox.gmm$n - length(coef(cox.gmm))
results[5,1] <- exp(mean(simex.boot))
results[5,2] <- exp(mean(simex.boot) - 1.96 * sd(simex.boot))
results[5,3] <- exp(mean(simex.boot) + 1.96 * sd(simex.boot))
results[5,4] <- 2 * pt(abs(mean(simex.boot)/sd(simex.boot)), df, lower.tail = F) 

row.names(results) <- c("Zhan classes - Naïve","GMM - Naïve", "GMM Fuzzy","GMM - Simex (Average MC)", "GMM - Simex (Full bootstrap)")
...
tab.cox.mm <- Hmisc::latex(resultsLatex,
             title = "",
             file = "Output/Tables/cox.mm.fuzzy.tex",
