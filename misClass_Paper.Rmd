...
results[1,4] <- cox.gmm.sum$coefficients[5]

# Simex
results[2,1] <- exp(cox.gmm.simex$coefficients)[1]
...
results[2,4] <- cox.gmm.simex.sum$coefficients$jackknife[4]
...
results[3,4] <- 2 * pt(abs(mean(simex.boot)/sd(simex.boot)), df, lower.tail = F) 

row.names(results) <- c("Naive", "Simex", "Simex Bootstrap")
...
tab.cox.mm <- Hmisc::latex(resultsLatex,
             ...
             caption = "Results from MC-SIMEX corrected cox regression",
             label = "tab.cox.mm")
