...
conf.GMM <- table(tmp$class,tmp$clust)
...
```{r}
### Use most likely groups from confusion matrix
tmp$gmm.class <- row.names(conf.GMM)[tmp$clust]
table(tmp$class, tmp$gmm.class)

tmp$OS <- Surv(tmp$eventtime,tmp$event)
fit1 <- survfit(tmp$OS~tmp$class)
fit2 <- survfit(tmp$OS~tmp$gmm.class)
names(fit1$strata) <- gsub("class=", "", names(fit1$strata))
names(fit2$strata) <- gsub("gmm.class=", "", names(fit2$strata))
plot1 <- ggsurvplot(fit1, data = tmp, title = "Zhan Classes", pval = T)
plot2 <- ggsurvplot(fit2, data = tmp, title = "GMM Classes", pval = T)
...
tmp.test <- merge(pData(zhan_proc),test.labs2,by = 0)
tmp.test$gmm.class <- row.names(conf.GMM)[tmp.test$clust]
table(tmp.test$class, tmp.test$gmm.class)
```

KM plots in test data
```{r}
tmp.test$OS <- Surv(tmp.test$eventtime,tmp.test$event)
fit1 <- survfit(tmp.test$OS~tmp.test$class)
fit2 <- survfit(tmp.test$OS~tmp.test$gmm.class)
names(fit1$strata) <- gsub("class=", "", names(fit1$strata))
names(fit2$strata) <- gsub("gmm.class=", "", names(fit2$strata))
plot1 <- ggsurvplot(fit1, data = tmp.test, title = "Zhan Classes (Test data)", pval = T)
plot2 <- ggsurvplot(fit2, data = tmp.test, title = "GMM Classes (Test data)", pval = T)
arrange_ggsurvplots(list(plot1, plot2))
```
...
```{r}
tmp$zhan.risk <- factor(ifelse(tmp$class %in% c("PR", "MF", "MS"), "High Risk", "Low Risk"),
                            levels = c("Low Risk", "High Risk"))
tmp$gmm.risk <- factor(ifelse(tmp$gmm.class %in% c("PR", "MF", "MS"), "High Risk", "Low Risk"),
                           levels = c("Low Risk", "High Risk"))

coxph(OS~zhan.risk, data = tmp)
coxph(OS~gmm.risk, data = tmp)
```

Kaplan-Meier plot for low vs high risk
```{r}
fit1 <- survfit(tmp$OS~tmp$zhan.risk)
fit2 <- survfit(tmp$OS~tmp$gmm.risk)
names(fit1$strata) <- gsub("zhan.risk=", "", names(fit1$strata))
names(fit2$strata) <- gsub("gmm.risk=", "", names(fit2$strata))
plot1 <- ggsurvplot(fit1, data = tmp, title = "Zhan Risk Class", pval = T)
plot2 <- ggsurvplot(fit2, data = tmp, title = "GMM Risk Class", pval = T)
arrange_ggsurvplots(list(plot1, plot2))
```
...
Try in test data
```{r}
tmp.test$zhan.risk <- factor(ifelse(tmp.test$class %in% c("PR", "MF", "MS"), "High Risk", "Low Risk"),
                            levels = c("Low Risk", "High Risk"))
tmp.test$gmm.risk <- factor(ifelse(tmp.test$gmm.class %in% c("PR", "MF", "MS"), "High Risk", "Low Risk"),
                           levels = c("Low Risk", "High Risk"))

coxph(OS~zhan.risk, data = tmp.test)
coxph(OS~gmm.risk, data = tmp.test)
```

Kaplan-Meier plot for low vs high risk
```{r}
fit1 <- survfit(tmp.test$OS~tmp.test$zhan.risk)
fit2 <- survfit(tmp.test$OS~tmp.test$gmm.risk)
names(fit1$strata) <- gsub("zhan.risk=", "", names(fit1$strata))
names(fit2$strata) <- gsub("gmm.risk=", "", names(fit2$strata))
plot1 <- ggsurvplot(fit1, data = tmp.test, title = "Zhan Risk Class (test data)", pval = T)
plot2 <- ggsurvplot(fit2, data = tmp.test, title = "GMM Risk Class (test data)", pval = T)
arrange_ggsurvplots(list(plot1, plot2))
```
...
if(!file.exists("GeneratedData/boots.GMM.RData")){
  # Set up parallel computations
  registerDoParallel(cores = 30)
  ...
  save(r.boot, file = "GeneratedData/boots.GMM.RData")
  stopImplicitCluster()
} else load("GeneratedData/boots.GMM.RData")
...
```{r}
cox.gmm  <- coxph(OS~gmm.risk, model = TRUE, data = tmp)
...
# Naive
results[1,1] <- cox.gmm.sum$coefficients[2]
results[1,2] <- cox.gmm.sum$conf.int[3]
results[1,3] <- cox.gmm.sum$conf.int[4]

# Simex
results[2,1] <- exp(cox.gmm.simex$coefficients)
results[2,2] <- exp(cox.gmm.simex$coefficients -1.96 * sqrt(cox.gmm.simex$variance.jackknife))
results[2,3] <- exp(cox.gmm.simex$coefficients +1.96 * sqrt(cox.gmm.simex$variance.jackknife))

# Simex bootstrap
results[3,1] <- exp(mean(simex.boot))
results[3,2] <- exp(mean(simex.boot) - 1.96 * sd(simex.boot))
results[3,3] <- exp(mean(simex.boot) + 1.96 * sd(simex.boot))

row.names(results) <- c("Naive", "Simex", "Simex BS")
colnames(results)  <- c("HR", "Lower 95", "Upper 95")
kable(results, digits = 2,
      caption = "Hazard ratio of high vs low risk with the naive and simex corrected model")
```

### Try with 7 classes
```{r, echo = T, eval = T}
# chunk disabled
tmp$gmm.class <- as.factor(tmp$gmm.class)
model <- coxph(OS ~ gmm.class, data = tmp, model = T)
