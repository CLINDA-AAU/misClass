...
summarizeResultsCox <- function(results, HR = 2, digits = 2){
  results <- exp(results)
  ## Bias
  bias <- colMeans(results - HR, na.rm = TRUE)
  ## MSE
  mse <- colMeans((results - HR)^2, na.rm = TRUE)
  return(round(rbind(bias,mse), digits))
}
...
coxTable <- cbind(rep(nSample, each = 2),
...
tCox <- Hmisc::latex(coxTable[,-1],
             ...
             caption = "Results from cox simulations",
...
rownames(hatPi) <- 1:7 #Only rename rows, columns are original classes and seem okay
