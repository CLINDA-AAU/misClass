...
kable(coxTable, caption = "Simulation results with Cox for misclassification probability of 0.1, 0.2 or 0.3")
```
...
plot1 <- ggsurvplot(fit1, data = tmp, title = "Zhan Classes")
plot2 <- ggsurvplot(fit2, data = tmp, title = "GMM Classes")
...
names(fit1$strata) <- gsub("class=", "", names(fit1$strata))
names(fit2$strata) <- gsub("gmm.class=", "", names(fit2$strata))
plot1 <- ggsurvplot(fit1, data = tmp.test, title = "Zhan Classes (Test data)")
plot2 <- ggsurvplot(fit2, data = tmp.test, title = "GMM Classes (Test data)")
arrange_ggsurvplots(list(plot1, plot2))
```
...
# chunk disabled
newClass <- data.frame(newClass = factor(fit.GMM$classification))
row.names(newClass) <- gsub("U133Plus-","",row.names(newClass)) %>% 
  gsub(" \\(.*","",.) %>% 
  gsub("-.*","",.) %>%
  gsub("P0","P",.) %>%
  strsplit(., "_") %>%
...
pheno.mm2 <- merge(pheno.mm, newClass, by.x = "PatientID", by.y = 0, all.x = T)
pheno.mm2$surv <- with(pheno.mm2, Surv(eventtime, event))

model <- coxph(surv ~ newClass, data = pheno.mm2, model = T)

model.simex <- mcsimex(model,
                       SIMEXvariable = "newClass",
                       mc.matrix = xhatPi,
                       asymptotic = F)
```
