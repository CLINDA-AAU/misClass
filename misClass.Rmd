...
# fitted by Mclust by Monte Carlo integration.
Pi <- function(fit){
...
summariseResults(results, alpha = alpha, beta = beta)
...
if(!file.exists("results_survival.Rdata")){
for(i in 1:51){
  print(i)
  y = j[,i]
  ...
  save(res.surv, file = "results_survival.Rdata")
}

load("results_survival.Rdata")
...
#  names(ctpred.outofbag) <- rownames(pred.outofbag$z)
  ...
  save(r.boot, file = "boots.Rdata")
}else load("boots.Rdata")
...
if(!file.exists("results_gse4581.Rdata")){
  for(i in 1:51){
    print(i)
    yx = j[,i]
  ...
  save(res, file = "results_gse4581.Rdata")
} else load("results_gse4581.Rdata")
...
if(!file.exists("fit.mcsimex.Rdata")){
  fit.mcsimex <- mcsimex(fit.poi, SIMEXvariable = "class", mc.matrix = xhatPi,
                         jackknife.estimation = FALSE)
  save(fit.mcsimex, file = "fit.mcsimex.Rdata")
} else load("fit.mcsimex.Rdata")
