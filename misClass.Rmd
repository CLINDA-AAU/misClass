...
p <- ggdendrogram(as.dendrogram(clusters), labels=F, leaf_labels = F) + 
...
plotdata <- data.frame("PC1" = pca.mm[[2]][,1],
                       "PC2" = pca.mm[[2]][,2],
                       "Class" = factor(fit$classification))
ggplot(plotdata, aes(x = PC1, y = PC2, col = Class)) +
...
fit.km.plot <- ggsurvplot(fit.km,
           data = pheno.mm,
           title = "Training data")
fit.km.plot$plot <- fit.km.plot$plot +
